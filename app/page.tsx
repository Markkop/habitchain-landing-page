...
export default function LandingPage() {
  ...
  const text = isPt
    ? {
        ...
        steps: [
          ...
          },
          {
            ...
            description:
              "Complete seu ciclo para recuperar sua aposta e ganhar recompensas extras em pools patrocinados.",
          },
        ],
        showcaseTitleTop: "Um dashboard simples para",
    ...
    : {
        ...
        heroBadge: "The Web3 Habit Builder",
        heroTitle: "Stake on Yourself.",
        heroDescription:
          "Build lasting habits with real financial stakes. Commit to your goals, self-attest your check-ins recorded on-chain, and earn rewards for your consistency.",
        startBuilding: "Start Building Today",
        learnMore: "Learn More",
        howTitle: "Discipline in Three Steps",
        howDescription:
          "We combine the psychology of loss aversion with blockchain transparency to make sure you stick to your goals.",
        steps: [
          {
            title: "1. Commit & Stake",
            description:
        ...
        showcaseTitleTop: "A simple dashboard for",
        showcaseTitleHighlight: "consistent results.",
        showcaseBullets: [
        ...
        previewHabitName: "Morning Meditation",
        previewCountdownContext: "check-in window",
        previewCardOneName: "Daily Workout",
        previewCardTwoName: "Read 20 Pages",
        previewCardTwoTimeLeft: "Ready",
        ...
        launchNow: "Launch the App Now",
        footerBuiltOnBase: "Built on Base.",
        footerEmail: "Email",
        closeModal: "Close",
        modalTitle: "Join the Waitlist",
        modalDescription: "Be the first to know when we launch!",
  ...
  return (
    <>
      ...
      <div lang={language === "pt-BR" ? "pt-BR" : "en"} className="flex flex-col min-h-screen bg-background text-foreground">
      {/* Navigation */}
      <header className="px-6 h-16 flex items-center justify-between border-b border-border/40 backdrop-blur-sm sticky top-0 z-50">
        ...
        <nav className="hidden md:flex gap-6 text-sm font-medium text-muted-foreground">
          <Link href="#how-it-works" className="hover:text-foreground transition-colors">{text.navHowItWorks}</Link>
          <Link href="#features" className="hover:text-foreground transition-colors">{text.navFeatures}</Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {isShiftHeld ? (
            <Button asChild size="sm" className="rounded-full px-5">
              <a href="https://app.habitchain.xyz">{text.launchApp}</a>
            </Button>
          ) : (
      ...
      <main className="flex-1">
        ...
        <section className="px-6 py-20 md:py-32 flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            {isShiftHeld ? (
              <Button asChild size="lg" className="rounded-full text-lg px-8 h-14 bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 shadow-xl shadow-primary/20">
                <a href="https://app.habitchain.xyz" className="flex items-center gap-2">
                  {text.startBuilding} <ArrowRight className="w-5 h-5" />
                </a>
              </Button>
            ) : (
            ...
            <Button asChild variant="outline" size="lg" className="rounded-full text-lg px-8 h-14 border-2">
              <Link href="#how-it-works">{text.learnMore}</Link>
            </Button>
          </div>
        </section>
        ...
        <section id="how-it-works" className="scroll-mt-24 px-6 py-24 bg-secondary/30 border-y border-border/40">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold mb-4">{text.howTitle}</h2>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <Card className="bg-card/50 border-2 border-border/50 hover:border-primary/30 transition-all group">
                <CardHeader>
                  ...
                  <CardTitle>{text.steps[0].title}</CardTitle>
              ...
              <Card className="bg-card/50 border-2 border-border/50 hover:border-primary/30 transition-all group">
                <CardHeader>
                  ...
                  <CardTitle>{text.steps[1].title}</CardTitle>
                  <CardDescription className="text-base pt-2">
              ...
              <Card className="bg-card/50 border-2 border-border/50 hover:border-primary/30 transition-all group">
                <CardHeader>
                  ...
                  <CardTitle>{text.steps[2].title}</CardTitle>
                  <CardDescription className="text-base pt-2">
        ...
        <FeaturesCarouselSection language={language} onJoinWaitlist={() => setShowWaitlistForm(true)} />
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight">
                {text.showcaseTitleTop} <br />
                <span className="text-primary">{text.showcaseTitleHighlight}</span>
              </h2>
              <ul className="space-y-4">
                {text.showcaseBullets.map((item, i) => (
            ...
            <div className="relative">
              ...
              <div className="relative space-y-4">
                <InteractiveHabitDemo
                  language={language}
                  habitName={text.previewHabitName}
                  stake="$5.00"
                  initialCheckIns={12}
                  initialCountdownSeconds={14 * 60 * 60 + 23 * 60}
                  countdownContextLabel={text.previewCountdownContext}
                  showJoinWaitlistAction={false}
                  className="scale-105 shadow-2xl z-10"
                />
                <HabitPreviewCard 
                  language={language}
                  name={text.previewCardOneName} 
                  stake="$15.00" 
                  checkIns={24} 
                  timeLeft="0s" 
                  status="done"
                ...
                <HabitPreviewCard 
                  language={language}
                  name={text.previewCardTwoName} 
                  stake="$2.50" 
                  checkIns={5} 
                  timeLeft={text.previewCardTwoTimeLeft} 
                  status="ready"
        ...
        <section className="px-6 py-24 bg-gradient-to-b from-transparent to-primary/5">
          <div className="max-w-4xl mx-auto text-center rounded-3xl p-12 border-2 border-primary/20 bg-card/50 shadow-2xl relative overflow-hidden">
            ...
            <h2 className="text-4xl font-bold mb-6">{text.ctaTitle}</h2>
            ...
            {isShiftHeld ? (
              <Button asChild size="lg" className="rounded-full text-lg px-10 h-14 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20">
                <a href="https://app.habitchain.xyz">{text.launchNow}</a>
              </Button>
            ) : (
        ...
        <FaqSection language={language} onJoinWaitlist={() => setShowWaitlistForm(true)} />
      </main>
      ...
      <footer className="px-6 py-12 border-t border-border/40 text-center text-sm text-muted-foreground">
        ...
        <p>&copy; {new Date().getFullYear()} HabitChain. {text.footerBuiltOnBase}</p>
        <div className="flex justify-center gap-6 mt-6">
          ...
          <a href="mailto:contact@habitchain.xyz" className="hover:text-foreground">{text.footerEmail}</a>
        </div>
      </footer>
      ...
      {showWaitlistForm && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-2xl font-bold mb-4">{text.modalTitle}</h2>
            <p className="text-muted-foreground mb-6">{text.modalDescription}</p>
            <form 
